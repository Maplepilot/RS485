...
double omega_target1 = 0.0;
double omega_target2 = 0.0;
double omega_target3 = 0.0;
double omega_target4 = 0.0;
double omega_target5 = 0.0;
double omega_target6 = 0.0;
double omega_target7 = 0.0;
double omega_target8 = 0.0;
...
{       
  if((millis()-lastMilli) >= LOOPTIME)   
                    {                                    // enter tmed loop
                        dT = millis()-lastMilli;
                        lastMilli = millis();
                        sendCmd(omega_target1, slave1);
                        
                        vel_msg1.x = read_slave(slave1);
                        p1.publish(&vel_msg1);
                        delay(1);
                        sendCmd(omega_target2, slave2);
                        
                        vel_msg2.x = read_slave(slave2);
                        p2.publish(&vel_msg2);
                        delay(1);
                        sendCmd(omega_target3, slave3);
                        
                        vel_msg3.x = read_slave(slave3);
                        p3.publish(&vel_msg3);
                        delay(1);
                        sendCmd(omega_target4, slave4);
                        
                        vel_msg4.x = read_slave(slave4);
                        p4.publish(&vel_msg4);
                        delay(1);
                        sendCmd(omega_target5, slave5);
                        
                        vel_msg5.x = read_slave(slave5);
                        p5.publish(&vel_msg5);
                        delay(1);
                        sendCmd(omega_target6, slave6);
                        
                        vel_msg6.x = read_slave(slave6);
                        p6.publish(&vel_msg6);
                        delay(1);
                        sendCmd(omega_target7, slave7);
                        
                        vel_msg7.x = read_slave(slave7);
                        p7.publish(&vel_msg7);
                        delay(1);
                        sendCmd(omega_target8, slave8);
...
void sendCmd(double omega_target,int slave)
{
  int target_send;
  digitalWrite(8, RS485Transmit); //DE,RE=HIGH, TX enabled
  target_send = int(omega_target/0.00031434064); //convert rad/s to 16 bit integer to send
  ...
  Serial1.write(sT); Serial1.write(sA); Serial1.write(sH); Serial1.write(sL); 
  Serial1.write(sH1); Serial1.write(sL1); Serial1.write(sH2); Serial1.write(sL2);
  Serial1.write(sH3);
  Serial1.write(sP);
...
void printMotorInfo()  
{                                                                      
   Serial.print(" target1:");                  Serial.print(omega_target1);
   //Serial.print(" actual1:");                  Serial.print(omega_actual1);
   //Serial.print(" actual:");                  Serial.print(omega_actual);
   Serial.println();
   /*Serial.print(" target2:");                  Serial.print(omega_target2);
